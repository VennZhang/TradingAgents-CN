                ...
                try:
                    ...
                    if not username or not password:
                        ...
                        if env_username and env_password:
                            ...
                            if env_host:
                                host = env_host
                                # 🔥 Docker Compose 环境下，将 localhost 替换为 mongodb
                                # 仅当 .env 中 MONGODB_HOST 明确为 "mongodb" 时才替换，
                                # 避免 Docker 容器内直接运行时误替换
                                if is_docker and host == 'localhost' and env_host == 'mongodb':
                                    host = 'mongodb'
                                    logger.info(f"🐳 检测到 Docker Compose 环境，将 host 从 localhost 改为 mongodb")
                ...
                try:
                    ...
                    if not password:
                        ...
                        if env_password:
                            ...
                            if env_host:
                                host = env_host
                                # 🔥 Docker Compose 环境下，将 localhost 替换为 redis
                                # 仅当 .env 中 REDIS_HOST 明确为 "redis" 时才替换，
                                # 避免 Docker 容器内直接运行时误替换
                                if is_docker and host == 'localhost' and env_host == 'redis':
                                    host = 'redis'
                                    logger.info(f"🐳 检测到 Docker Compose 环境，将 Redis host 从 localhost 改为 redis")
