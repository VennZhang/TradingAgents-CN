...
class DataSourceManager:
    ...
    def get_index_data(self, symbol: str, start_date: str, end_date: str) -> str:
        try:
            from .providers.china.index_data import get_index_data_provider
            provider = get_index_data_provider()
            df = provider.get_index_daily(symbol, start_date, end_date)
            if df is not None and not df.empty:
                recent = df.tail(60)
                lines = [f"指数: {symbol} 日线数据"]
                first_d = str(recent.index[0]) if hasattr(recent.index[0], 'strftime') else str(recent.iloc[0].get('trade_date', ''))
                last_d = str(recent.index[-1]) if hasattr(recent.index[-1], 'strftime') else str(recent.iloc[-1].get('trade_date', ''))
                lines.append(f"数据区间: {first_d} 至 {last_d}")
                lines.append("-" * 40)
                for _, row in recent.iterrows():
                    date_str = str(row.get("trade_date", str(row.name)))
                    o = row.get("open", row.get("开盘价", "N/A"))
                    h = row.get("high", row.get("最高价", "N/A"))
                    lx = row.get("low", row.get("最低价", "N/A"))
                    c = row.get("close", row.get("收盘价", "N/A"))
                    v = row.get("vol", row.get("成交量", "N/A"))
                    chg = row.get("pct_chg", row.get("涨跌幅", "N/A"))
                    lines.append(f"{date_str} | O:{o} H:{h} L:{lx} C:{c} V:{v} Chg:{chg}%")
                return "\n".join(lines)
            return f"指数 {symbol} 无数据"
        except Exception as e:
            logger.warning(f"获取指数数据失败 {symbol}: {e}")
            return f"获取指数 {symbol} 数据失败: {e}"

    def get_index_constituents(self, symbol: str) -> str:
        try:
            from .providers.china.index_data import get_index_data_provider
            provider = get_index_data_provider()
            constituents = provider.get_index_constituents(symbol)
            if constituents:
                lines = [f"指数 {symbol} 成分股及权重:"]
                lines.append("-" * 40)
                total_w = 0.0
                for i, c in enumerate(constituents[:20]):
                    w = c.get("weight", 0)
                    total_w += w
                    lines.append(f"{i+1}. {c.get('con_code', 'N/A')} | 权重: {w:.2f}%")
                if len(constituents) > 20:
                    lines.append(f"... (共 {len(constituents)} 只成分股)")
                lines.append(f"前20权重合计: {total_w:.2f}%")
                return "\n".join(lines)
            return f"指数 {symbol} 无成分股数据"
        except Exception as e:
            logger.warning(f"获取指数成分股失败 {symbol}: {e}")
            return f"获取指数 {symbol} 成分股失败: {e}"

    def get_sector_list(self) -> str:
        try:
            from .providers.china.sector_data import get_sector_data_provider
            ...
            if sectors:
                ...
                for i, s in enumerate(sectors):
                    lines.append(f"{i+1}. {s['sector_code']} - {s['sector_name']}")
                return "\n".join(lines)
            return "无板块数据"
        except Exception as e:
    ...
    def get_sector_performance(self, start_date: str, end_date: str) -> str:
        try:
            from .providers.china.sector_data import get_sector_data_provider
            provider = get_sector_data_provider()
            sectors = provider.get_sw_sector_list()
            ...
            lines.append("-" * 40)
            for s in sectors[:10]:
                df = provider.get_sector_daily(s["sector_code"], start_date, end_date)
                if df is not None and not df.empty:
                    first_c = df.iloc[0].get("close", df.iloc[0].get("收盘价", 0))
                    last_c = df.iloc[-1].get("close", df.iloc[-1].get("收盘价", 0))
                    if first_c and last_c:
                        try:
                            pct = (float(last_c) / float(first_c) - 1) * 100
                            lines.append(f"{s['sector_name']}: {pct:+.2f}%")
                        except (ValueError, ZeroDivisionError):
                            pass
            lines.append(f"(共 {len(sectors)} 个板块)")
            return "\n".join(lines)
        except Exception as e:
            logger.warning(f"获取板块表现失败: {e}")
            return f"获取板块表现失败: {e}"

    def get_index_sector_weights(self, index_code: str) -> str:
        try:
            from .providers.china.index_data import get_index_data_provider
            provider = get_index_data_provider()
            weights = provider.get_index_sector_weights(index_code)
            if weights:
                lines = [f"指数 {index_code} 板块权重分布:"]
                lines.append("-" * 40)
                for name, w in weights.items():
                    lines.append(f"  {name}: {w}%")
                return "\n".join(lines)
            return f"指数 {index_code} 无板块权重数据"
        except Exception as e:
            logger.warning(f"获取指数板块权重失败 {index_code}: {e}")
