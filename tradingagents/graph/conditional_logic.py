...
class ConditionalLogic:
    ...
    def should_continue_tech(self, state) -> str:
        current_count = state.get("tech_tool_call_count", 0)
        report = state.get("index_technical_report", "")
        messages = state.get("messages", [])
        last_msg = messages[-1] if messages else None
        if current_count >= 3:
            return "msg_clear_tech"
    ...
    def should_continue_sentiment(self, state) -> str:
        current_count = state.get("sentiment_tool_call_count", 0)
        report = state.get("index_sentiment_report", "")
        messages = state.get("messages", [])
        last_msg = messages[-1] if messages else None
        if current_count >= 3:
            return "msg_clear_sentiment"
        if report and len(report) > 100:
            return "msg_clear_sentiment"
        if hasattr(last_msg, 'tool_calls') and last_msg.tool_calls:
            return "tool_sentiment"
        return "msg_clear_sentiment"

    def should_continue_macro_policy(self, state) -> str:
        current_count = state.get("macro_tool_call_count", 0)
        report = state.get("macro_policy_report", "")
        messages = state.get("messages", [])
        last_msg = messages[-1] if messages else None
        if current_count >= 3:
            return "msg_clear_macro"
        if report and len(report) > 100:
            return "msg_clear_macro"
        if hasattr(last_msg, 'tool_calls') and last_msg.tool_calls:
            return "tool_macro"
    ...
    def should_continue_valuation(self, state) -> str:
        current_count = state.get("valuation_tool_call_count", 0)
        report = state.get("index_valuation_report", "")
        messages = state.get("messages", [])
        last_msg = messages[-1] if messages else None
        if current_count >= 3:
            return "msg_clear_valuation"
        if report and len(report) > 100:
            return "msg_clear_valuation"
        if hasattr(last_msg, 'tool_calls') and last_msg.tool_calls:
            return "tool_valuation"
        return "msg_clear_valuation"

    def should_continue_sector(self, state) -> str:
        current_count = state.get("sector_tool_call_count", 0)
        report = state.get("sector_theme_report", "")
        messages = state.get("messages", [])
        last_msg = messages[-1] if messages else None
        if current_count >= 3:
            return "msg_clear_sector"
        if report and len(report) > 100:
            return "msg_clear_sector"
        if hasattr(last_msg, 'tool_calls') and last_msg.tool_calls:
            return "tool_sector"
    ...
    def should_continue_index_impact(self, state) -> str:
        current_count = state.get("impact_tool_call_count", 0)
        report = state.get("index_impact_report", "")
        messages = state.get("messages", [])
        last_msg = messages[-1] if messages else None
        if current_count >= 3:
            return "msg_clear_impact"
        if report and len(report) > 100:
            return "msg_clear_impact"
        if hasattr(last_msg, 'tool_calls') and last_msg.tool_calls:
            return "tool_impact"
    ...
    def should_continue_index_debate(self, state) -> str:
        ...
        if current_count >= max_count:
            return "Index Research Manager"
        next_speaker = "Index Bear Researcher" if current_speaker.startswith("多头") else "Index Bull Researcher"
        return next_speaker

    def should_continue_index_risk(self, state) -> str:
        ...
        if current_count >= max_count:
            return "Index Risk Manager"
        if latest_speaker == "risky":
            return "Index Safe Debater"
        elif latest_speaker == "safe":
            return "Index Neutral Debater"
        return "Index Risky Debater"
