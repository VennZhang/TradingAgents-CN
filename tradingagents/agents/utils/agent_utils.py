...
"""
    ...
    @tool
    def get_index_data(
        index_code: Annotated[str, "指数代码，如 000300.SH (沪深300), 000016.SH (上证50), 000905.SH (中证500), 000852.SH (中证1000), 000688.SH (科创50), 399006.SZ (创业板指)"],
        start_date: Annotated[str, "开始日期，格式 YYYY-MM-DD"],
        end_date: Annotated[str, "结束日期，格式 YYYY-MM-DD"]
    ) -> str:
        """获取A股宽基指数的日线行情数据（K线、涨跌幅、成交量等）。"""
        try:
    ...
    def get_index_constituents(
        index_code: Annotated[str, "指数代码，如 000300.SH"]
    ) -> str:
        """获取指数的成分股列表及权重数据。"""
        try:
            from tradingagents.dataflows.interface import get_index_constituents_unified
            return get_index_constituents_unified(index_code)
        except Exception as e:
            return f"获取指数成分股失败: {e}"

    @staticmethod
    @tool
    def get_index_sector_weights(
        index_code: Annotated[str, "指数代码，如 000300.SH"]
    ) -> str:
        """获取指定宽基指数的板块权重分布（申万行业级别）。"""
        try:
            from tradingagents.dataflows.interface import get_index_sector_weights_unified
            return get_index_sector_weights_unified(index_code)
        except Exception as e:
            return f"获取指数板块权重失败: {e}"

    @staticmethod
    @tool
    def get_sector_data(
        sector_name: Annotated[str, "板块名称，如 '银行', '新能源', '食品饮料'"]
    ) -> str:
        """获取指定板块的最近行情表现数据。"""
        try:
            from tradingagents.dataflows.interface import get_sector_list_unified, get_sector_performance_unified
            ...
            sectors = provider.get_sw_sector_list()
            sector_daily = pd.DataFrame()
            for s in sectors:
                if s["sector_name"] == sector_name or sector_name.lower() in s["sector_name"].lower():
                    from datetime import datetime, timedelta
                    end = datetime.now().strftime("%Y-%m-%d")
                    start = (datetime.now() - timedelta(days=60)).strftime("%Y-%m-%d")
                    sector_daily = provider.get_sector_daily(s["sector_code"], start, end)
                    if sector_daily is not None and not sector_daily.empty:
                        break
            if sector_daily is not None and not sector_daily.empty:
                return f"板块 {sector_name} 最近表现:\n{sector_daily.tail(30).to_string()}"
            return get_sector_performance_unified(
    ...
    def get_sector_performance(
        ...
        """获取所有申万行业板块在指定区间的涨跌幅表现。"""
        try:
            from tradingagents.dataflows.interface import get_sector_performance_unified
            return get_sector_performance_unified(start_date, end_date)
        except Exception as e:
            return f"获取板块表现失败: {e}"
    ...
    def get_sector_list() -> str:
        """获取申万一级行业板块列表（31个行业）。"""
        try:
            from tradingagents.dataflows.interface import get_sector_list_unified
            return get_sector_list_unified()
        except Exception as e:
            return f"获取板块列表失败: {e}"

    @staticmethod
    @tool
    def get_futures_sentiment_data(
        index_code: Annotated[str, "指数代码，如 000300.SH"]
    ) -> str:
        """获取股指期货持仓、基差、融资融券等资金情绪数据。"""
        try:
            index_name = {"000016.SH": "IH", "000300.SH": "IF", "000905.SH": "IC", "000852.SH": "IM"}
            name = index_name.get(index_code, index_code)
            lines = [f"# 期货持仓与情绪分析: {name}"]
            lines.append("**注意**: 股指期货基差和持仓数据需通过专业数据源获取")
            try:
                import akshare as ak
                df = ak.futures_zh_minute_sina(symbol="IF0")
            ...
            try:
                ...
                if df is not None and not df.empty:
                    ...
                    for _, row in recent.iterrows():
                        d = row.get("信用交易日期", row.get("date", ""))
                        buy = row.get("融资买入额", row.get("margin_buy", "N/A"))
                        lines.append(f"  {d} | 融资买入额: {buy}")
            except Exception:
                lines.append("  融资融券数据暂时不可用")
            return "\n".join(lines)
        except Exception as e:
            return f"获取期货情绪数据失败: {e}"

    @staticmethod
    @tool
    def get_fund_flow_data(
        market: Annotated[str, "市场: 'A股' 或 '港股'"]
    ) -> str:
        """获取北向资金流向、ETF资金流等数据。"""
        try:
            lines = ["# 资金流向分析"]
            ...
            try:
                ...
                lines.append("北向资金数据暂时不可用")
            try:
                import akshare as ak
                df = ak.fund_etf_category_sina(symbol="规模指数基金")
                if df is not None and not df.empty:
                    lines.append("\n## ETF资金流概览")
            ...
            return f"获取资金流数据失败: {e}"

    @staticmethod
    @tool
    def get_margin_data() -> str:
        """获取融资融券余额、融资买入额等杠杆情绪数据。"""
        try:
            lines = ["# 融资融券数据"]
            try:
                import akshare as ak
    ...
    @staticmethod
    @tool
    def get_index_valuation_data(
        index_code: Annotated[str, "指数代码，如 000300.SH"]
    ) -> str:
        """获取指数估值水平、PE/PB历史分位、成分股结构和板块权重等数据。"""
        try:
            from tradingagents.dataflows.interface import get_index_data_unified, get_index_constituents_unified
            index_names = {
            ...
            lines = [f"# 指数估值分析: {name} ({index_code})"]
            lines.append("\n## 历史PE/PB分位")
            lines.append("**注意**: 完整PE/PB历史分位需通过Tushare或专业数据终端获取")
            try:
                index_data = get_index_data_unified(index_code,
                    (pd.Timestamp.now() - pd.DateOffset(days=365)).strftime("%Y-%m-%d"),
                    pd.Timestamp.now().strftime("%Y-%m-%d"))
                lines.append(f"\n{index_data[:500]}")
            except Exception:
                pass
            lines.append("\n## 成分股结构")
            const_data = get_index_constituents_unified(index_code)
            lines.append(f"\n{const_data[:500]}")
            lines.append("\n## 板块权重分布")
            from tradingagents.dataflows.interface import get_index_sector_weights_unified
            weight_data = get_index_sector_weights_unified(index_code)
            lines.append(f"\n{weight_data}")
            return "\n".join(lines)
        except Exception as e:
            return f"获取指数估值数据失败: {e}"
    ...
    def get_macro_news(
        ...
        """获取宏观经济指标（CPI、PMI等）和政策动向信息。"""
        try:
            lines = ["# 宏观政策与事件分析"]
            lines.append(f"区间: {start_date} 至 {end_date}")
            try:
                import akshare as ak
                lines.append("\n## 宏观经济指标")
                try:
                    ...
                    if cpi is not None and not cpi.empty:
                        lines.append(f"\n### CPI (最近): {cpi.tail(3).to_string()}")
                except Exception:
                    pass
                try:
                    pmi = ak.macro_china_pmi_yearly()
                    if pmi is not None and not pmi.empty:
                        lines.append(f"\n### PMI (最近): {pmi.tail(3).to_string()}")
                except Exception:
                    pass
            except Exception:
                lines.append("宏观经济数据需联网获取")
            lines.append("\n## 近期政策动向")
            lines.append("注: 完整政策新闻分析需接入新闻源API")
            lines.append("请结合以下公开信息进行判断:")
            lines.append("- 人民银行公开市场操作公告 (www.pbc.gov.cn)")
            lines.append("- 国务院常务会议新闻 (www.gov.cn)")
            lines.append("- 中央财经委会议纪要")
            return "\n".join(lines)
        except Exception as e:
            return f"获取宏观新闻失败: {e}"
